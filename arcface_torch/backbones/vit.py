...
class VisionTransformer_Mix(nn.Module):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=(1, num_patches))
        self.layer = nn.Sequential(nn.Conv1d(512, 512, 1),
                                    nn.PReLU(512))
    ...
    def forward(self, x, y):
        ...
        # ===========disentangle=============
        B = x.shape[0]
        y = torch.reshape(y, (B, self.num_patches, self.embed_dim))
        ...
        y = self.avgpool(y)
        z = y.transpose(1, 2)
        z = self.batchmatmul(x, z) # z:B*d*p(_x) & B*1*d(y_hat) -> B*1*p
        y = self.batchmatmul(z, y) # B*d*1(_y) & B*1*p(z) -> B*d*p
        y = x - y
        y = self.layer(y)
        x = x + y
        # ===========end base================
        # attn and base could replace seamlessly
        # ===========attn====================
